...
func (alert *AlertPegoutFeeNotReset) Check(dataSource AlertDataSource) (Severity, Description, Values, error) {
	...
	if severity > SEVERITY_OK {
		...
		description = fmt.Sprintf(
			"Pegout transaction already has %d (pegoutBlockHeight %d, lastConfirmedBlockHeight %d) confirmations but the fee has not been reset (nextSvb = %d). Pegout: %s. Bitcoin TX: %s. Steps to resolve: %s",
