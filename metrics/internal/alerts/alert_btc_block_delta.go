...
func (alert *AlertBtcBlockDelta) Check(dataSource AlertDataSource) (Severity, Description, Values, error) {
	...
	if alert.severity > SEVERITY_OK {
		alert.description = Description(
			fmt.Sprintf(
				"There is a block-height delta of %d between the BitcoinClient contract (height %d: %d blocks + %d confirmations) and the Bitcoin network (height %d). Runbook url: %s",
				delta,
				blockHeightContract,
