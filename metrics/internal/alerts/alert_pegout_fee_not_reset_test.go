	...
	tests := []TestDesc{
		...
		},
		{
			...
			Expect: TestResWant{
				...
				Description: Description(fmt.Sprintf("Pegout transaction already has 1 (pegoutBlockHeight 1234567, lastConfirmedBlockHeight 1234568) confirmations but the fee has not been reset (nextSvb = 123). Pegout: %s. Bitcoin TX: %s. Runbook url: %s", tonUrl, btcUrl, runbookUrl)),
				Err:         nil,
			},
		},
	}
