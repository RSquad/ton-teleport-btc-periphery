...
func TestAlertDkgEvicted(t *testing.T) {
	tests := []TestDesc{
		{
			Name: "SEVERITY_OK (DKG with 0% evicted)",
		...
		{
			Name: "SEVERITY_WARNING (DKG with 10% evicted)",
			DataSource: NewAlertDataSourceTesting(AlertDataSourceTestingConfig{
				DkgDbFn: func() (*coordinator.DKG, error) {
					vset := make(coordinator.VSet, 10)
					for i := range 10 {
		...
		{
			Name: "SEVERITY_CRITICAL (DKG with 20% evicted)",
			DataSource: NewAlertDataSourceTesting(AlertDataSourceTestingConfig{
				DkgDbFn: func() (*coordinator.DKG, error) {
					vset := make(coordinator.VSet, 10)
					for i := range 10 {
	...
	DoAlertTests(t, tests, NewAlertDkgEvicted())
}
