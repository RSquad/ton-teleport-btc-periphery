...
func DeserializeDkgR2(r2Packages map[uint16][]byte /*map[FROM]data*/, vSetMask *big.Int) (
	...
	for _, fromValidatorIdx := range ExtractSortedKeysFromMap(r2Packages) {
		...
		if len(toValidatorData) != int(vSetSize-1 /*ourself*/) {
			return nil, true, fromValidatorIdx, fmt.Errorf("valid package count %d from Validator %d does not equal vSetSize - 1 (we don't send a package to ourselves): expected %d", len(toValidatorData), fromValidatorIdx, vSetSize)
		}
